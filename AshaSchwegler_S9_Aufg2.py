...
def AshaSchwegler_S9_Aufg2(A,squiggle_A,b,squiggle_b): 
   ...
   if checkCond(A, squiggle_A) == True:
      rel_Fehler = ((cond_A/(1-(cond_A*squiggle_A_inf/A_inf)))*((squiggle_A_inf/A_inf)+(squiggle_b_inf/b_inf)))
      result_ohneFehler = np.linalg.solve(A,b)
      result_mitFehler = np.linalg.solve(squiggle_A,squiggle_b)
      return rel_Fehler, result_ohneFehler, result_mitFehler
   else: 
      return "NaN"
